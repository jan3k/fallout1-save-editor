...
F12se groups editing around save metadata, globals/quests, map/SAV files, world map, inventory/items, stats, skills/perks, kills, player status and raw/hex operations. `f1se` now mirrors the useful grouping while keeping Fallout 1 source-derived field validation and round-trip byte preservation.
