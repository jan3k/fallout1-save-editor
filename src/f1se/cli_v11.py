...
from f1se.project.features import filter_features
...
def _cmd_features(argv: list[str]) -> int:
    ...
    for row in rows:
        ...
        print(f"{row.category:20s} | {row.name:38s} | f1se={row.f1se_status:12s} | risk={row.risk_level:10s} | interface={interface} | {row.notes}")
    return 0
