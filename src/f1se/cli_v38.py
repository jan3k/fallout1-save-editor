...
from f1se.cli_v37 import main as previous_main
from f1se.project.special_write import set_special
...
def _cmd_special_set(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="f1se special-set")
    parser.add_argument("slot")
    parser.add_argument("stat", help="strength/str, perception/per, endurance/end, charisma/cha, intelligence/int, agility/agi, luck/luc")
    parser.add_argument("value")
    parser.add_argument("--game", choices=GAME_CHOICES, default="auto")
    parser.add_argument("--mode", choices=("raw", "semantic"), default="semantic", help="Fallout 1 only; semantic recalculates stable derived stats")
    ...
    if isinstance(field, dict):
        ...
        field_name = field
    print(f"{mode}: {payload['game_kind']} {payload.get('stat')} {payload.get('old_value', payload.get('diff', {}).get('old_value'))} -> {payload.get('new_value', payload.get('diff', {}).get('new_value'))} ({field_name})")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args and args[0] == "special-set":
        return _cmd_special_set(args[1:])
