...
class PatchFieldSummary:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"name": self.name, "risk": self.risk, "offset": self.offset, "offset_hex": f"0x{self.offset:X}", "old_value": self.old_value, "new_value": self.new_value, "old_bytes": self.old_bytes, "new_bytes": self.new_bytes}
...
class PatchRiskSummary:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"changed_field_count": self.changed_field_count, "risks": list(self.risks), "contains_advanced": self.contains_advanced, "safe_only": self.safe_only, "requires_advanced_confirmation": self.requires_advanced_confirmation, "backup_before_write": True, "atomic_write": True, "fields": [field.to_dict() for field in self.fields]}
...
class DirtyState:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"dirty": self.dirty, "changed_field_count": self.changed_field_count, "risks": list(self.risks), "safe_only": self.safe_only, "contains_advanced": self.contains_advanced}
...
class SaveEditorSession:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {"slot_path": str(self.slot_path), "size": len(self.save_dat.data), "size_hex": f"0x{len(self.save_dat.data):X}", "sha256": self.save_dat.sha256, "signature": h.signature, "version": h.version, "player_name": h.player_name, "save_name": h.save_name, "saved_date": f"{h.real_year:04d}-{h.real_month:02d}-{h.real_day:02d}", "current_map_file": h.current_map_file, "map_id": h.map_id, "elevation": h.elevation, "function5_start": self.save_dat.player_object.start, "function6_start": self.save_dat.critter_stats.start, "warnings": list(dict.fromkeys(self.save_dat.warnings))}
    ...
    def field_groups(self) -> dict[str, list[str]]:
        ...
        return {"special_base": [n for n in names if n.startswith("player.base_") and n.split("player.base_", 1)[1] in {"strength", "perception", "endurance", "charisma", "intelligence", "agility", "luck"}], "special_bonus": [n for n in names if n.startswith("player.bonus_") and n.split("player.bonus_", 1)[1] in {"strength", "perception", "endurance", "charisma", "intelligence", "agility", "luck"}], "skills": [n for n in names if n.startswith("skills.")], "tag_skills": [n for n in names if n.startswith("tag_skills.")], "traits": ["traits.trait_0", "traits.trait_1"], "perks": [n for n in names if n.startswith("perks.")], "kill_counts": [n for n in names if n.startswith("kill_counts.")], "inventory": [n for n in names if n.startswith("inventory.")], "options": [n for n in names if n.startswith("options.")], "pc": [n for n in names if n.startswith("pc.")], "derived_stats": [n for n in names if n.startswith("player.") and self.save_dat.fields[n].risk == "ADVANCED"], "player_safe": [n for n in names if n in {"player.current_hp", "player.radiation", "player.poison", "player.crippled_body_parts"}]}
    ...
    def globals_scan_payload(self) -> dict[str, Any]:
        ...
        for row in discover_global_state_candidates(self.save_dat.data, self.save_dat.blocks):
            doc = row.to_dict()
            doc["labels"] = labels_by_block(row.block_index)
            rows.append(doc)
        return {"slot_path": str(self.slot_path), "candidates": rows}

    def global_labels_payload(self, block_index: int | None = None) -> dict[str, Any]:
        return project_global_labels_payload(block_index)
    ...
    def validation_summary(self) -> dict[str, Any]:
        ...
        return {"status": status, "issues": issues, "warnings": warnings, "artifact_warnings": artifact_warnings, "raw_block_warnings": raw_warnings, "map_scan_warnings": map_warnings}
    ...
    def patch_risk_summary(self, patch: dict[str, int]) -> PatchRiskSummary:
        ...
        for name, value in patch.items():
            ...
            fields.append(PatchFieldSummary(name=name, risk=field.risk, offset=field.abs_offset, old_value=int(field.value), new_value=int(value), old_bytes=old_bytes.hex(), new_bytes=new_bytes.hex()))
        ...
        return PatchRiskSummary(changed_field_count=len(fields), risks=ordered_risks, contains_advanced=contains_advanced, safe_only=safe_only, requires_advanced_confirmation=contains_advanced, fields=fields)

    def dirty_state(self, patch: dict[str, int]) -> DirtyState:
        summary = self.patch_risk_summary(patch)
        return DirtyState(dirty=summary.changed_field_count > 0, changed_field_count=summary.changed_field_count, risks=summary.risks, safe_only=summary.safe_only, contains_advanced=summary.contains_advanced)
